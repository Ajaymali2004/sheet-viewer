...
const PAGE_SIZE = 20;
const DUMMY_IMAGE = "https://play-lh.googleusercontent.com/W64nQFWLRJ39k9q4VFEQeLMizDiN5QBDcs8WCbxiGtmYqAXFQX2gzfKX4V8Dvd_2xXs";
...
export default function SheetTable({ columns, rows, columnTypes, filters, onFilterChange }: SheetTableProps) {
    ...
    const isFilterVisible = (col: string) => {
        return openFilters.has(col) || hasActiveFilter(col);
    };
    ...
    return (
        <div className="w-full flex justify-center pb-16">
            <div
                className="relative overflow-x-auto rounded-xl border border-gray-700"
            ...
            >
                <table className="text-sm text-left border-collapse" style={{ minWidth: "50vw" }}>
                                                ...
                                                {type === "number" ? (
                                                    <div className="flex gap-1 items-center">
                                                        <input
                                                            ...
                                                            onChange={(e) =>
                                                                onFilterChange(col, { ...filters[col], min: e.target.value })
                                                            }
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e) =>
                                                                onFilterChange(col, { ...filters[col], max: e.target.value })
                                                            }
                                                            className="w-20 px-2 py-1 bg-gray-800 border border-gray-600 rounded-lg text-white text-xs focus:outline-none focus:border-blue-500"
                                                        />
                                                        {active && (
                                                            <button
                                                                ...
                                                                className="text-gray-400 hover:text-gray-200 text-sm font-bold leading-none"
                                                                title="Clear filter"
                                                            >
                                                                ✕
                                                            </button>
                                                    ...
                                                    <div className="flex gap-1 items-center">
                                                        <input
                                                            ...
                                                            onChange={(e) =>
                                                                onFilterChange(col, { ...filters[col], text: e.target.value })
                                                            }
                    ...
                    <tbody>
                        {visibleRows.map((row, rowIndex) => (
                            ...
                            >
                                {!hasBuiltInSerial && (
                                    <td className="px-4 py-3 text-gray-600 font-mono text-xs">{rowIndex + 1}</td>
                                )}
                                {columns.map((col, colIndex) => {
                                    const type = columnTypes[col];
                                    const value = row[col];
                ...
                {visibleCount < rows.length && (
                    <div ref={loaderRef} className="text-center py-6 text-gray-500 text-sm">
                        Loading more rows...
                    </div>
                )}
